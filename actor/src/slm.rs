...
pub fn create<T: Clone + Send + 'static>(
    ...
    let (model_path, tokenizer_path, eos_tokens, num_kv_heads, head_dim) = match model {
        ...
        Model::Llama33b => (
            LLAMA3_3B_MODEL_PATH,
            LLAMA3_3B_TOKENIZER_PATH,
            LLAMA3_3B_EOS_TOKENS,
            LLAMA3_3B_NUM_KV_HEADS,
            LLAMA3_3B_HEAD_DIM,
        ),
        Model::Llama38b => (
            LLAMA3_8B_MODEL_PATH,
            LLAMA3_8B_TOKENIZER_PATH,
            LLAMA3_8B_EOS_TOKENS,
            LLAMA3_8B_NUM_KV_HEADS,
            LLAMA3_8B_HEAD_DIM,
        ),
        Model::Gemma34b => (
            GEMMA3_4B_MODEL_PATH,
            GEMMA3_4B_TOKENIZER_PATH,
            GEMMA3_4B_EOS_TOKENS,
            GEMMA3_4B_NUM_KV_HEADS,
            GEMMA3_4B_HEAD_DIM,
        ),
        Model::Smollm3 => (
            SMOLLM3_MODEL_PATH,
            SMOLLM3_TOKENIZER_PATH,
            SMOLLM3_EOS_TOKENS,
            SMOLLM3_NUM_KV_HEADS,
            SMOLLM3_HEAD_DIM,
        ),
    };
        ...
        move || {
            ...
            while let Ok(input) = input_rx.recv() {
                ...
                for _ in 0..MAX_TOKENS {
                    ...
                    let cache_position_tensor = if let Model::Gemma34b = model {
                        Some(onnx::Value::from_slice::<i64>(&session.onnx, &[positions.len()], &positions))
                    } else {
                        None
                    };
