...
async fn main() {
    ...
    let model = match choice {
        1 => llm::Model::Phi3,
        2 => llm::Model::Llama33b,
        3 => llm::Model::Llama38b,
        4 => llm::Model::Gemma34b,
        5 => llm::Model::Smollm3,
    ...
    let (llm_handle, mut llm_listener) = llm::create::<()>(&onnx, onnx::Executor::Cuda(0), model, &epoch);
    ...
    let mut history = history::ChatHistory::new();
    ...
    loop {
        ...
        let mut input = String::new();
        io::stdin().read_line(&mut input).unwrap();
        history.add(history::Role::User(0), input.trim().to_string());
        let prompt = prompt::build(model, identity, &personality, tools, facts, &history);
        //println!("({})", prompt);
        ...
        loop {
            match llm_listener.recv().await {
                llm::Output::Token { payload: _, token } => {
                    print!("{}", token);
                    io::stdout().flush().unwrap();
                    response.push_str(&token);
                }
                llm::Output::Eos { payload: _ } => {
                    println!();
                    break;
                }
            }
        }
        history.add(history::Role::Robot, response);
    }
}
